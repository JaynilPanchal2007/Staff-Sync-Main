...
function parseStaffRows(text) {
    ...
    for (const line of lines) {
        ...
        if (/^(sr\.?|no\.?|s\.?n|employee|faculty|staff|name|dept|department|qualification|roll|email|day|lect)\b/i.test(line) &&
            line.split(/[,\t|]/).length > 2)
            continue;
        ...
        if (!/^[A-Za-z][A-Za-z\s.'-]{2,}$/.test(name))
            continue;
        const possibleId = parts.find((p) => /^[A-Z0-9_-]{2,12}$/i.test(p) && !/^[A-Za-z\s]+$/.test(p)) ||
            `EMP-${100 + idCounter++}`;
        const dept = parts.find((p) => /computer|science|math|physics|english|mechanical|electrical|operations|assembly|quality|commerce|chemistry|biology|history|geography|civil|it\b|software|hr|management/i.test(p)) ||
            parts[2] ||
            'General';
        const role = parts.find((p) => /professor|assistant|teacher|faculty|lead|engineer|technician|operator|lecturer|principal|hod|head/i.test(p)) || 'Staff Member';
        rows.push({
            name,
...
function parseTimetableRows(text) {
    const rows = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^(day|time|period|subject|room|faculty|slot)\b/i.test(line))
            continue;
        ...
        if (timeMatch) {
            const day = DAYS_ORDER.find((d) => line.toLowerCase().includes(d.toLowerCase())) || currentDay;
            const subject = parts.find((p) => !DAYS_ORDER.some((d) => p.toLowerCase().includes(d.toLowerCase())) &&
                !/^\d{1,2}:\d{2}$/.test(p) &&
                !/^(room|hall|lab|lh|cr)\b/i.test(p) &&
                p.length > 2) || parts[1] || 'Lecture';
...
pdfRouter.post('/pdf/import', upload.single('file'), async (req, res) => {
    try {
        ...
        try {
            ...
            console.error('[PDF ERROR] name   :', parseErr?.name);
            console.error('[PDF ERROR] message:', parseErr?.message);
            console.error('[PDF ERROR] stack  :\n', parseErr?.stack);
            fs.unlinkSync(filePath);
            return res.status(400).json({
                error: `Failed to parse PDF: ${parseErr.message}. Ensure the file is not corrupted or password-protected.`,
            });
        }
        ...
        if (!text.trim()) {
            fs.unlinkSync(filePath);
            return res.status(400).json({
                error: 'No readable text could be extracted. The PDF may be an image-only scan. Please use a text-based PDF.',
            });
        }
        const lines = text
            .split('\n')
            .map((l) => l.trim())
            .filter((l) => l.length > 2);
        let effectiveType = req.body.type || 'staff';
        let extractedRows = effectiveType === 'staff' ? parseStaffRows(text) : parseTimetableRows(text);
