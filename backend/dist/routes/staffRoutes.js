    ...
    try {
        ...
        if (isRealFilter(q)) {
            ...
            staff = staff.filter((s) => s.name?.toLowerCase().includes(search) ||
                ...
                (s.subjects && s.subjects.some((sub) => sub.toLowerCase().includes(search))) ||
...
staffRouter.post('/staff', async (req, res) => {
    try {
        const admin = resolveAdmin(req);
        const orgId = resolveOrgId(req);
        const { name, employeeId, email, phone, password, department, role, subjects, classes, skills, certifications, qualification, experience, workingDays, availability, maxWorkload, maxWeeklyHours, status, } = req.body;
...
staffRouter.put('/staff/:id', (req, res) => {
    try {
        ...
        if (updates.role)
            updates.role = String(updates.role).trim();
        if (updates.email)
...
staffRouter.get('/dashboard/faculty/:id', (req, res) => {
    try {
        ...
        const attendance = dbService.getAttendance(orgId, today).find((a) => a.staffId === staff.id || a.staffId === staff.employeeId);
        ...
        const acceptedProxies = proxyRequests.filter((p) => staffIds.includes(p.targetStaffId) && p.status === 'accepted');
        const acceptedTodayProxies = acceptedProxies.filter((p) => p.workDetails?.date === today || p.workDetails?.day?.toLowerCase() === dayName.toLowerCase());
        ...
        const combinedTodaySchedule = [...todaySchedule, ...proxyLecturesToday].sort((a, b) => (a.startTime || '').localeCompare(b.startTime || ''));
