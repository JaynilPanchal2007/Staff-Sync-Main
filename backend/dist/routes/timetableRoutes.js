...
timetableRouter.get('/timetable', (req, res) => {
    try {
        ...
        let list = dbService.getTimetable(orgId, facultyId);
        if (day && day !== 'all') {
    ...
    try {
        ...
        if (room && String(room).trim() && String(room).trim().toLowerCase() !== 'tba') {
            const roomConflict = dbService
                .getTimetable(orgId)
                .find((t) => t.day?.toLowerCase() === day.toLowerCase() &&
...
timetableRouter.put('/timetable/:id', (req, res) => {
    try {
        ...
        if (targetRoom && targetRoom.toLowerCase() !== 'tba') {
            const roomConflict = dbService
                .getTimetable(orgId)
                .find((t) => (t.id || t.timetableId) !== currentId &&
...
timetableRouter.post('/timetable/exchange', (req, res) => {
    try {
        ...
        const overlap = (a, b) => Math.max(0, Math.min(toMinutes(a.endTime), toMinutes(b.endTime)) - Math.max(toMinutes(a.startTime), toMinutes(b.startTime)));
        ...
        const targetSlot = pool.sort((a, b) => sameDay.length > 0 ? overlap(source, b) - overlap(source, a) : Math.abs(toMinutes(a.startTime) - toMinutes(source.startTime)) - Math.abs(toMinutes(b.startTime) - toMinutes(source.startTime)))[0];
