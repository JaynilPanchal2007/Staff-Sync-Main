...
replacementRouter.get('/replacements/gaps', (req, res) => {
    try {
        ...
        const gaps = dbService.getGaps(orgId, status);
        res.json({ gaps });
    }
    catch (err) {
...
replacementRouter.put('/proxy/:id/accept', (req, res) => {
    try {
        ...
        if (proxyReq.workDetails?.affectedType === 'lecture' || proxyReq.workDetails?.subject) {
            ...
            const matchingEntry = timetable.find((t) => (t.facultyId === proxyReq.fromStaffId || t.facultyName === proxyReq.fromStaffName) &&
            ...
            const matchingShift = shifts.find((s) => (s.employeeId === proxyReq.fromStaffId || s.employeeName === proxyReq.fromStaffName) &&
