...
export interface RecommendationResult {
  ...
  bestCandidate: ReplacementCandidate | null;
  aiInsight?: string | null;
  timestamp: string;
}

export class SmartReplacementEngine {
  public async evaluateGap(gapId: string): Promise<RecommendationResult> {
    ...
    for (const staff of allStaff) {
      ...
      if (orgType === 'school' || orgType === 'college') {
        const staffLectures = allTimetable.filter(
          (t) => (t.facultyId === staff.id || t.facultyId === staff.employeeId || t.facultyId === staff.staffId) && t.day === gap.day
        );
      ...
      if (staff.availability && Array.isArray(staff.availability) && staff.availability.length > 0) {
        const dayAvailability = staff.availability.find(
          (av: any) => av.day?.toLowerCase() === gap.day?.toLowerCase()
        );
      ...
      if (orgType === 'school' || orgType === 'college') {
        ...
        const staffSubs = (staff.subjects || []).map((s: string) => s.toLowerCase().trim());
        ...
        const reqSkills: string[] = Array.isArray(gap.requiredSkills)
          ? gap.requiredSkills
          : (gap.requiredSkillsOrSubject || '').split(',').map((s: string) => s.trim().toLowerCase()).filter(Boolean);

        const staffSkills: string[] = (staff.skills || []).map((s: string) => s.toLowerCase().trim());
        const staffCerts: string[] = (staff.certifications || []).map((c: string) => c.toLowerCase().trim());
      ...
      const staffProxies = proxyRequests.filter(
        (p) => (p.targetStaffId === staff.id || p.targetStaffId === staff.employeeId || p.targetStaffId === staff.staffId) && p.status === 'accepted'
      ).length;
