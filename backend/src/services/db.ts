...
export class DatabaseService {
  ...
  private initDefaultSeedData() {
    ...
    const makeAvail = (days: string[]) => days.map((day) => ({ day, available: true }));
  ...
  private async initMongo() {
    ...
    if (uri && uri.trim().length > 0) {
      try {
        ...
        console.warn('[StaffSync Backend] MongoDB connection attempted but failed. Using in-memory fallback engine:', (err as Error).message);
  ...
  public async connectMongo(uri: string): Promise<{ success: boolean; message: string }> {
    try {
  ...
  public getAdminByOrg(orgId: string) {
    if (!orgId) return null;
    return (
      this.dbCache.admins.find(
        (a) => a.organizationId === orgId || (a.organizationType === 'global' && a.id === 'admin_darshan_patel')
      ) || null
    );
  }
  ...
  public updateAdmin(updates: Partial<typeof this.dbCache.admin>) {
  ...
  public saveOrganization(org: any) {
  ...
  // Staff
  public getStaff(orgId?: string) {
    if (!orgId) return this.dbCache.staff;
  ...
  public addStaff(staffMember: any) {
  ...
  public updateStaff(id: string, updates: any) {
  ...
  // Attendance
  public getAttendance(orgId?: string, date?: string) {
    let list = this.dbCache.attendance;
    if (orgId) list = list.filter((a) => a.organizationId === orgId);
    if (date) list = list.filter((a) => a.date === date);
    return list;
  }

  public setAttendance(record: any) {
    const idx = this.dbCache.attendance.findIndex(
  ...
  // Timetable
  public getTimetable(orgId?: string, facultyId?: string) {
    let list = this.dbCache.timetable;
    if (orgId) list = list.filter((t) => t.organizationId === orgId);
    if (facultyId && facultyId !== 'all') {
      const cleanFid = facultyId.trim().toLowerCase();
      list = list.filter(
        (t) =>
          t.facultyId?.toLowerCase() === cleanFid ||
  ...
  public addTimetableEntry(entry: any) {
  ...
  public updateTimetableEntry(id: string, updates: any) {
  ...
  public getTimetableEntryById(id: string) {
    return this.dbCache.timetable.find((t) => t.id === id || t.timetableId === id) || null;
  }

  // Exchange requests
  public getExchanges(orgId?: string, facultyId?: string, status?: string) {
    let list = this.dbCache.exchanges;
    if (orgId) list = list.filter((x) => x.organizationId === orgId);
    if (facultyId) {
      list = list.filter(
        (x) => x.sourceFacultyId === facultyId || x.targetFacultyId === facultyId
      );
    }
  ...
  public addExchangeRequest(data: any) {
  ...
  public updateExchangeRequest(id: string, updates: any) {
  ...
  // Shifts
  public getShifts(orgId?: string, employeeId?: string, date?: string) {
    let list = this.dbCache.shifts;
    if (orgId) list = list.filter((s) => s.organizationId === orgId);
    if (employeeId) list = list.filter((s) => s.employeeId === employeeId);
    if (date) list = list.filter((s) => s.date === date);
    return list;
  }

  public addShift(shift: any) {
  ...
  public updateShift(id: string, updates: any) {
  ...
  // Staffing Gaps
  public getGaps(orgId?: string, status?: string) {
    let list = this.dbCache.gaps;
    if (orgId) list = list.filter((g) => g.organizationId === orgId);
    if (status) list = list.filter((g) => g.status === status);
    return list;
  }

  public getGapById(id: string) {
    return this.dbCache.gaps.find((g) => g.id === id || g.gapId === id) || null;
  }

  public addGap(gap: any) {
  ...
  public updateGap(id: string, updates: any) {
  ...
  // Proxy Requests
  public getProxyRequests(orgId?: string, staffId?: string) {
    let list = this.dbCache.proxyRequests;
    if (orgId) list = list.filter((p) => p.organizationId === orgId);
    if (staffId) {
  ...
  public updateProxyRequest(id: string, updates: any) {
  ...
  // Notifications
  public getNotifications(orgId?: string, staffId?: string) {
    let list = this.dbCache.notifications;
    if (orgId) list = list.filter((n) => n.organizationId === orgId);
    if (staffId) {
  ...
  public markAllNotificationsRead(orgId?: string, staffId?: string) {
    ...
    if (this.isMongoConnected) {
      ...
      if (staffId) query.targetStaffId = staffId;
      NotificationModel.updateMany(query, { read: true }).catch(console.error);
    }
  }

  // Audit Logs
  public getAuditLogs(orgId?: string, limit = 100) {
    let list = this.dbCache.auditLogs;
    if (orgId) list = list.filter((l) => !l.organizationId || l.organizationId === orgId);
