...
export async function askStaffSyncAssistant(orgId: string | undefined, query: string): Promise<string> {
...
- Keep the response clear, professional, factual, and directly actionable for the administrator.`;
      ...
      console.warn('Gemini call failed or timed out, falling back to deterministic DB assistant:', (err as Error).message);
    }
  }
  ...
  if (q.includes('lowest workload') || q.includes('least work')) {
    if (staff.length === 0) return `I don't have enough data to answer that. No staff records exist yet in the database.`;
    const sorted = [...staff].sort(
      (a, b) => (a.assignedLectures || a.weeklyHours || 0) - (b.assignedLectures || b.weeklyHours || 0)
    );
...
export async function getSmartAdjustmentAiInsight(gap: any, candidates: any[]): Promise<string | null> {
  const ai = getGemini();
...
- Keep it highly professional, factual, and direct.`;
    ...
    console.warn('Gemini Smart Adjustment insight call failed:', (err as Error).message);
  }
  return null;
}
